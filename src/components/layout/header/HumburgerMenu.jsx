...
function HumburgerMenu({scroll}) {
    ...
    useEffect(() => {
        function handlerClose(e) {
            if (e.target.closest(".humburger-toggler") || (e.target.closest(".top"))) {
                return;
            } else {
                setIsOpen(false);
            }
        }
        window.addEventListener("click", handlerClose, true)
        return () => {
    ...
    return (
        <div className={`humbergerMenu bg-[#f8efd8] min-[949px]:hidden fixed top-0 left-0 w-full z-50`}>
            <div className="top px-4! flex justify-between items-center py-7! border-b border-px border-[#b92e2d]">
                ...
                <div className="wraper flex items-center gap-6!">
                   <Cart />
                    <div
                    className="humburger-toggler w-12 h-12 flex flex-col justify-center items-center cursor-pointer"
                    onClick={handlerToggle}
                >
                    <span
                        className={`absolute h-0.75 w-8 bg-black transition-all duration-300 ${isOpen ? "rotate-45" : "-translate-y-2"
                            }`}
                    ></span>

                    <span
                        className={`absolute h-0.75 w-8 bg-black transition-all duration-300 ${isOpen ? "opacity-0" : ""
                            }`}
                    ></span>
            ...
            <div className={` flex absolute top-26.1 bg-[#f8efd8] z-50 left-0 w-full flex-col overflow-hidden gap-2 ${isOpen ? "h-59" : "h-0"} transition-all duration-300`}>
                ...
                <Link to="/blog" className="menu-item luckiest-guy tracking-wider p-2!">BLOG</Link>
                <Link to="/contact" className="menu-item luckiest-guy tracking-wider p-2!">CONTACT</Link>
